...
template <>
class insert_thread<std::unordered_map<std::string, uint32_t>> {
 public:
  typedef typename std::unordered_map<std::string, uint32_t> Table;
  typedef typename std::vector<std::string>::iterator it_t;
  static void func(Table& table, it_t begin, it_t end) {
    for (; begin != end; begin++) table.insert(std::make_pair(*begin, 0));
  }
};

template <>
class insert_thread<std::unordered_map<uint32_t, uint32_t>> {
 public:
  typedef typename std::unordered_map<uint32_t, uint32_t> Table;
  typedef typename std::vector<uint32_t>::iterator it_t;
  static void func(Table& table, it_t begin, it_t end) {
    for (; begin != end; begin++) table.insert(std::make_pair(*begin, 0));
  }
};
...
template <>
class read_thread<std::unordered_map<std::string, uint32_t>> {
 public:
  typedef typename std::unordered_map<std::string, uint32_t> Table;
  typedef typename std::vector<typename Table::key_type>::iterator it_t;
  static void func(Table& table, it_t begin, it_t end,
...
template <>
class read_thread<std::unordered_map<uint32_t, uint32_t>> {
 public:
  typedef typename std::unordered_map<uint32_t, uint32_t> Table;
  typedef typename std::vector<typename Table::key_type>::iterator it_t;
  static void func(Table& table, it_t begin, it_t end,
...
template <>
class read_insert_thread<std::unordered_map<std::string, uint32_t>> {
 public:
  typedef typename std::unordered_map<std::string, uint32_t> Table;
  typedef typename std::vector<typename Table::key_type>::iterator it_t;
  static void func(Table& table, it_t begin, it_t end,
    ...
    while (inserter_it != end) {
      if (dist(gen) < insert_prob) {
        // Do an insert
        table.insert(std::make_pair(*inserter_it, 0));
        ++inserter_it;
      } else {
        // Do a read
...
template <>
class read_insert_thread<std::unordered_map<uint32_t, uint32_t>> {
 public:
  typedef typename std::unordered_map<uint32_t, uint32_t> Table;
  typedef typename std::vector<typename Table::key_type>::iterator it_t;
  static void func(Table& table, it_t begin, it_t end,
