...
void ReadThroughputTest(ReadEnvironment<T>* env) {
  ...
  std::cout << "Throughput: " << std::fixed << counter.load() / (double)test_len
            << " reads/sec" << std::endl;
}

int main(int argc, char** argv) {
  const char* args[] = {"--power", "--thread-num", "--load", "--time",
                        "--seed"};
  ...
  const char* flag_help[] = {
      ...
      "If set, the benchmark uses cuckoohash_map, otherwise unordered_map"};
  parse_flags(argc, argv, "A benchmark for reads", args, arg_vars, arg_help,
