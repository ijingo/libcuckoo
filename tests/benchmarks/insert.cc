...
// Whether to use cuckoohash or unordered_map
...
void InsertThroughputTest(InsertEnvironment<T>* env) {
  ...
  size_t keys_per_thread =
      env->numkeys * ((end_load - begin_load) / 100.0) / thread_num;
  timeval t1, t2;
...
int main(int argc, char** argv) {
  ...
  size_t* arg_vars[] = {&power,      &table_capacity, &thread_num,
                        &begin_load, &end_load,       &seed};
  const char* arg_help[] = {
